...
function HomePage() {
	...
	const [furnitures, setFurnitures] = useLocalStorage<FurnitureType[]>(
		"furnitures",
		[]
	);
	const [categoryToRender, setCategoryToRender] = useState<string | null>(
		null
	);
	...
	return (
		<div className="flex flex-col">
			...
			<Modal
				isOpen={isOpen}
				onClose={toggleModal}
				title="Ajouter un nouveau meuble"
			>
				<NewFurnitureForm />
			</Modal>
			...
			<ul className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
				{furnitures
					.filter(
						(furniture) =>
							!categoryToRender ||
							furniture.category.name === categoryToRender
					)
					.map(
						({
							_id,
